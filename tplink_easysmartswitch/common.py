...
from .constants import LINK_STATUS_LOOKUP, POE_POWER_STATUS_LOOKUP, POE_PRIORITY_LOOKUP, POE_PD_CLASS_LOOKUP
...
def get_field_values_poe(source_str: str):
    ...
    poe_count_var_offset = source_str.find(b"poe_port_num")
    poe_count_equalsign_offset = source_str.find(b"=", poe_count_var_offset)
    poe_count_eol_offset = source_str.find(b";", poe_count_var_offset)
    poe_count = int(source_str[poe_count_equalsign_offset+1:poe_count_eol_offset])

    all_fields['poe_port_count'] = poe_count
...
def get_field_values_stats(source_str: str):
    port_count_var_offset = source_str.find(b"max_port_num")
    port_count_equalsign_offset = source_str.find(b"=", port_count_var_offset)
    port_count_eol_offset = source_str.find(b";", port_count_var_offset)
    port_count = int(source_str[port_count_equalsign_offset+1:port_count_eol_offset])

    all_fields = get_field_values(source_str)

    all_fields['port_count'] = port_count
...
class TPLinkSession:
    ...
    def get_stats(self):
        stats_url = self.build_url("PortStatisticsRpm.htm")
        stats_content = tplink_get(stats_url).content
        all_stats = get_field_values_stats(stats_content)
        return all_stats